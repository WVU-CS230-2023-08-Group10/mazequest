...
export {Collider, ExitIndicator, Tile};
...
class Tile extends Entity
{
     ...
     * @returns JSON string representation of Tile entity
     */
    serialize()
    {
        return '{ "type":"Tile", "name": "' + this._Name + '", "transform": ' + this._Transform.serialize() + ', "renderer": '
            + this._Renderer.serialize() + '}';
    }

    /**
     * Deserializes a JSON object into a Tile entity
     * @param {*} obj the JSON object
     * @param {*} game the current game instance
     * @returns 
     */
    static deserialize(obj, game)
    {
        return new Tile(obj.name, Transform.deserialize(obj.transform), Renderer.deserialize(obj.renderer, game.stage), game);
    }
}
